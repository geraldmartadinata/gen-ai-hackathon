...
<head>
    ...
    <title>Daftar Transaksi - NexStock</title>
        ...
        <div class="container mx-auto px-4">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="hidden md:flex items-center space-x-8">
                    ...
                    <a href="/admin/items" class="text-slate-600 hover:text-slate-900 transition-colors">Barang</a>
                    <a href="/admin/categories" class="text-slate-600 hover:text-slate-900 transition-colors">Kategori</a>
                    <a href="/admin/transactions" class="text-indigo-600 font-medium hover:text-indigo-700 transition-colors">Transaksi</a>
                </div>
    ...
    <main class="container mx-auto px-4 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <p class="text-slate-600 mt-1">Pantau semua pesanan yang masuk</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-lg font-semibold text-slate-900">Semua Transaksi</h2>
                <div class="flex items-center space-x-2">
                    <span class="text-sm text-slate-500">Menampilkan semua pesanan</span>
                </div>
            </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-slate-200">
                            ...
                            <th class="text-left py-3 px-4 text-xs font-medium text-slate-600 uppercase tracking-wider">Nama Pembeli</th>
                            <th class="text-left py-3 px-4 text-xs font-medium text-slate-600 uppercase tracking-wider">Tanggal Transaksi</th>
                            <th class="text-left py-3 px-4 text-xs font-medium text-slate-600 uppercase tracking-wider">Total Pembelian</th>
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="flex items-center space-x-3">
                                            <div class="w-8 h-8 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-lg flex items-center justify-center">
                                                <span class="text-blue-600 text-xs font-medium">{{ strtoupper(substr($invoice->user->name, 0, 1)) }}</span>
                                            </div>
                                            <div>
                                                <p class="text-sm font-medium text-slate-900">{{ $invoice->user->name }}</p>
                                                <p class="text-xs text-slate-500">{{ $invoice->user->email }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-3 px-4 text-sm text-slate-600">{{ $invoice->created_at->format('d M Y, H:i') }}</td>
                        ...
                        @else
                            <tr>
                                <td colspan="5" class="py-12 text-center">
                                    ...
                                    <h3 class="text-sm font-medium text-slate-900 mb-1">Belum ada transaksi</h3>
                                    <p class="text-sm text-slate-500">Transaksi akan muncul di sini saat user melakukan checkout.</p>
                                </td>
                            </tr>
                        @endif
                    </tbody>
